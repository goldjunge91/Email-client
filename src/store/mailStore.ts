...
interface MailStore {
  ...
  updateSmartInbox: (smartInboxId: string, updates: Partial<SmartInbox>) => void;
    ...
    persist(
      immer((set, get) => ({
        ...
        updateSmartInbox: (smartInboxId, updates) =>
          set((state) => {
            const index = state.smartInboxes.findIndex(s => s.id === smartInboxId);
            if (index !== -1) {
        ...
        removeSmartInbox: (smartInboxId) =>
          set((state) => {
            state.smartInboxes = state.smartInboxes.filter(s => s.id !== smartInboxId);
          }),

...
export const initializeMailIpc = () => {
  ...
  ipcMain.handle('mail:update-account', async (_event, accountId: string, updates: Partial<MailAccount>) => {
    try {
  ...
  ipcMain.handle('mail:disconnect-account', async (_event, accountId: string) => {
    try {
      const imapClient = imapClients.get(accountId);
      const smtpClient = smtpClients.get(accountId);
  ...
  // Mail Operations
  ipcMain.handle('mail:fetch-mails', async (_event, accountId: string, folderId: string, options?: any) => {
    try {
      const imapClient = imapClients.get(accountId);
      if (!imapClient || !imapClient.isReady()) {
        throw new Error('IMAP client not connected');
  ...
  ipcMain.handle('mail:mark-as-read', async (_event, accountId: string, mailId: string, read: boolean) => {
    try {
  ...
  ipcMain.handle('mail:mark-as-starred', async (_event, accountId: string, mailId: string, starred: boolean) => {
    try {
  ...
  ipcMain.handle('mail:move-mail', async (_event, accountId: string, mailId: string, targetFolderId: string) => {
    try {
  ...
  ipcMain.handle('mail:delete-mail', async (_event, accountId: string, mailId: string) => {
    try {
  ...
  // Send Mail
  ipcMain.handle('mail:send-mail', async (_event, accountId: string, mailOptions: any) => {
    try {
      const smtpClient = smtpClients.get(accountId);
      if (!smtpClient) {
  ...
  ipcMain.handle('mail:sync-folder', async (_event, accountId: string, folderId: string) => {
    try {
  ...
  // Search
  ipcMain.handle('mail:search', async (_event, query: string, options?: any) => {
    try {
  ...
  ipcMain.handle('mail:update-rule', async (_event, ruleId: string, updates: Partial<MailRule>) => {
    try {

...
import { ImapClient } from '../core/mail/protocols/imapClient';
import { SmtpClient } from '../core/mail/protocols/smtpClient';
...
export const initializeMailIpc = () => {
	...
	// Note: mailService is not an EventEmitter, so we don't forward events
	// Events will be handled through IPC responses or polling if needed
};
...
// 		async (_event, accountId: string, folderId: string) => {
// 			try {
// 				await mailService.syncFolder(accountId, folderId);
// 				return { success: true };
// 			} catch (error) {
// 				return { success: false, error: (error as Error).message };
// 			}
...
// 		async (_event, query: string, options?: any) => {
// 			try {
// 				const results = await mailService.search(query, options);
// 				return { success: true, data: results };
// 			} catch (error) {
// 				return { success: false, error: (error as Error).message };
// 			}
...
// 	ipcMain.handle('mail:apply-rules', async (_event, mailId: string) => {
// 		try {
// 			const mail = mailService.getMail(mailId);
// 			if (!mail) {
// 				throw new Error('Mail not found');
// 			}
...
// 	// TODO: Add event forwarding when mailService implements EventEmitter
// };

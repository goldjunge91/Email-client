...
export class RuleEngine extends EventEmitter {
  ...
  updateRule(ruleId: string, updates: Partial<MailRule>): void {
    ...
    if (rule) {
      ...
      if (updatedRule.conditions.type === 'javascript' && updatedRule.conditions.jsCode) {
        try {
          const fn = new Function('mail', 'context', updatedRule.conditions.jsCode);
  ...
  private evaluateSimpleCondition(mail: Mail, condition: MailRuleCondition): boolean {
    if (condition.type !== 'simple' || !condition.field || !condition.operator) {
      return false;
    }
  ...
  private evaluateJavaScriptCondition(
    ...
    try {
      ...
      const fn = this.jsRuleCache.get(condition.jsCode) || 
  ...
  // Action Execution
  private async executeAction(mail: Mail, action: MailRuleAction): Promise<void> {
    ...
    switch (action.type) {
      ...
      case 'notify':
        this.emit('notification:create', {
          mail,
          message: action.message || `New mail from ${mail.from[0]?.name || mail.from[0]?.address}`
        });
        break;
  ...
  async categorizeMail(mail: Mail): Promise<string> {
    ...
    if (subject.includes('invoice') || subject.includes('receipt') || 
    ...
    if (from.includes('github') || from.includes('gitlab') || 
        from.includes('jira') || from.includes('slack')) {
      return 'development';
    }

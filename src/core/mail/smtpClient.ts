...
export class SmtpClient extends EventEmitter {
  ...
  async connect(options: SmtpConnectionOptions, defaultFrom?: MailAddress): Promise<void> {
    try {
  ...
  async replyToMail(
    ...
    const replyOptions: SendMailOptions = {
      ...
      references: [
        ...(originalMail.references || []),
        originalMail.messageId
      ]
    };
  ...
  async forwardMail(
    ...
    const forwardHeader = `
---------- Forwarded message ----------
From: ${this.formatAddress(originalMail.from[0])}
...
`;
    ...
    const forwardOptions: SendMailOptions = {
      ...
      attachments: forward.includeAttachments ? originalMail.attachments : undefined
    };
  ...
  private formatAddresses(addresses: MailAddress[]): string[] {
    return addresses.map(addr => this.formatAddress(addr));
  }

  private formatAttachments(attachments?: MailAttachment[]): Mail.Attachment[] {
...
export const verifySmtpConnection = async (options: SmtpConnectionOptions): Promise<void> => {

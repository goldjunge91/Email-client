...
export class MailService extends EventEmitter {
  ...
  async updateAccount(accountId: string, updates: Partial<MailAccount>): Promise<void> {
  ...
  async deleteMails(mailIds: string[], permanent: boolean = false): Promise<void> {
    if (permanent) {
      // Permanently delete
      for (const mailId of mailIds) {
        const mail = this.mails.get(mailId);
      ...
      const trashFolder = this.getFoldersByAccount(account.id)
        .find(f => f.type === 'trash');
  ...
  private applyFilter(mails: Mail[], filter: MailFilter): Mail[] {
    return mails.filter(mail => {
      ...
      if (filter.isFlagged !== undefined && mail.isFlagged !== filter.isFlagged) {
        return false;
      }
      if (filter.hasAttachments !== undefined && mail.hasAttachments !== filter.hasAttachments) {
        return false;
      }
      ...
      if (filter.labels && !filter.labels.some(label => mail.labels?.includes(label))) {
        return false;
      }
  ...
  private updateFolderCounts(folderId: string): void {
    ...
    const folderMails = Array.from(this.mails.values())
      .filter(mail => mail.folderId === folderId);
  ...
  private updateSyncStatus(accountId: string, updates: Partial<MailSyncStatus>): void {
